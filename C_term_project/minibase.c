...
typedef struct Node
{
	...
	size_t key_num;    //the number of keys/records in current Node
...
void print_record(const Record *record){
	printf("record:%s%c%s\n", record->key, SEPARATER, record->value);
}
...
Node *insert_into_node_and_split(Node *root, Node *old_node, char *key, size_t index, Node *left_node, Node *right_node){
	char *extra_key;
	void *extra_pointer;
	size_t old_node_key_num;
	size_t new_node_key_num;
	// move elements for insertion.
...
void print_help(){
	...
	printf( "h                      print this help\n"
			"i <key><enter><value>  insert a record\n"
...
Record *parse_line(FILE *file, PARSE_LINE_ERROR_FLAG *error){
	...
	size_t comma = 0; 		//buff[comma-1] == SEPARATER
	*error = SUCCESS;
	while(*error == SUCCESS){
		ch = fgetc(file);
		switch (ch) {
	...
	while (scanf("%c", &command) != EOF) {
		...
		switch (command) {
			...
			case 'i':
				...
				if (error!=SUCCESS){
					printf("parse error\n"
						"key and value should be separated by comma. for example:\n"
						"name,passwordhash\n");
				}else{
					tStart = clock();
					insert_record(root, record);
					print_record(record);
			...
			case 's':
				...
				record = parse_line(stdin, &error);
				if (error!=SUCCESS){
					printf("type key to select end with a comma. for example:\n"
							"Turing,\n");
				}else{
					tStart = clock();
					record = find(root, record->key);
					if (!record) {
						printf("Not found\n");
					}else{
						print_record(record);
